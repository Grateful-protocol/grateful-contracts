...
module.exports.deploy = async function deploy(
  ...
  const { abis, info: deployInfo } = await hre.run(
    SUBTASK_GET_DEPLOYMENT_INFO,
    { instance }
  );
  ...
  console.log(
    "deploy info",
    await hre.run(SUBTASK_GET_DEPLOYMENT_INFO, { instance })
  );
  const contracts: any = Object.values(deployInfo.contracts).reduce(
    (contracts: any, c: any) => {
  ...
  contracts[prefix + "Router"].abi = await hre.run(SUBTASK_GET_MULTICALL_ABI, {
    info,
    instance,
  });
